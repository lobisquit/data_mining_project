...
\section{Clustering}
	...
	\subsection{Tecniche di Clustering}
		...
		\subsubsection{Hopkins Statistic}
			Dopo aver effettuato la vettorializzazione dei documenti, si \`{e} cercato di stimare l'effettiva tendenza al clustering 
			dei dati in nostro possesso.
			
			Per far ci\`{o} abbiamo utilizzato la statistica di Hopkins\cite{hopkins}.
			Il dataset randomizzato necessario per la comparazione \`{e} stato generato basandosi sul dominio 
			delle componenti dei vettori restituiti dalla vettorializzazione degli articoli mediante algoritmo \emph{Word2Vec}.
			
			Sono state eseguite svariate misurazioni di tale metrica utilizzando come sample una frazione da $0.1$ a $0.4$ dell'intero dataset.
			In tutti i test la statistica di Hopkins rilevata \`{e} stata compresa tra $0.011$ e $0.012$.
			
			In ogni caso i risultati testimoniano una forte tendenza al clustering dei dati in nostro possesso.
